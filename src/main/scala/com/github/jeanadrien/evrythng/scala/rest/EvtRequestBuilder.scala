...
trait EvtRequestBuilder {
    ...
    def get[T](url : String)(implicit reader : JsonReader[T]) = new EvtGetRequest[T](absUrl(url), authorization)
        .queryParameter(defaultQueryParams : _*)
    ...
    def getPage[T](url : String)(implicit reader : JsonReader[T]) = new EvtGetPageRequest[T](absUrl(url), authorization)
        .queryParameter(defaultQueryParams : _*)

    def nextPage[T](page : Page[T]) = page.nextPageUrl
        .map(url => new EvtGetPageRequest[T](url, authorization)(page.reader))
